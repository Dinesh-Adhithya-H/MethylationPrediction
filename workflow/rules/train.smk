...
rule get_model:
    input:
        training_py = "workflow/scripts/MachineLearning_Training.py",
        finalfile = OUTPUT_DIR + "combined_final_file.bed",
        annotation_file = config['METHYLATION_ANNOTATION']
    output:
        model = config['MODEL']
    shell:
        "python {input.training_py} {output.model} {input.finalfile} {input.annotation_file}"
